---
title: "Writing Presidential Farewells with GPT-2"
date: 2020-05-14
summary: "In this project I use GPT-2, a then-cutting edge language model, to analyze farewell addresses from 3 U.S. Presidents."
image: "featured.jpg"
---

Hello! This post is a final project write-up for the class [_The American Presidents and the Western Tradition_ (CLAS1120U)](https://bulletin.brown.edu/the-college/concentrations/clas/index.html?find=CLAS%201120U) at Brown University. I assume no computer science background when I present content on this page.

## Intro

GPT-2, which stands for Generative Pretrained Transformer 2, is a cutting-edge English language model developed and released by research group [OpenAI](https://openai.com/) in 2019. This model generates realistic passages of English text given any arbitrary prompt. In fact, GPT-2's output could be so convincing that the model's creators declined to publish the [actual model](https://github.com/openai/gpt-2) until months after the [original paper](https://openai.com/blog/better-language-models/) for fear that it would be used to overwhelm social media with hard-to-filter disinformation and propaganda.

Now we can find many fun projects on the internet that use the power of GPT-2:

- [This Word Does not Exist](https://www.thisworddoesnotexist.com/): a random dictionary entry generator that creates fake English words and assigns them fake (but not unreasonable) definitions
- [AI Dungeon](https://aidungeon.io/): an open-world choose-your-adventure style game that generates a plot on the fly according to typed responses from players
...
We will use the [farewell addresses](https://www.presidency.ucsb.edu/documents/presidential-documents-archive-guidebook/farewell-addresses-washington-1796-jackson-1837) of the following presidents:

- [Andrew Jackson](https://www.presidency.ucsb.edu/documents/farewell-address-0) -- 1837
- [Richard Nixon](https://www.presidency.ucsb.edu/documents/remarks-departure-from-the-white-house) (farewell to White House staff) -- 1974
- [Ronald Reagan](https://www.presidency.ucsb.edu/documents/farewell-address-the-nation) -- 1989

## How GPT-2 Works

GPT-2 is a transformer-based language model. This means that the way it generates each word of its output depends primarily on the presence of each word in the input or prompt. In terms of basic math, you can think of this as each word of the output is determined by a weighted sum of the input words (and we have a way to represent the concept of each word as a series of numbers). This concept is called _attention_ because each of the words of the output pay attention to each of the words of the input. Transformers are models that almost entirely rely on attention.

Below is a visualization of attention heads presented in the paper [_Attention Is All You Need_](https://arxiv.org/pdf/1706.03762.pdf). The opacities of the lines correspond to the amount of "attention" each attention head has on the other words.

![Attention visualized](attention.jpg)
...
Here's an example of [text generated](https://openai.com/blog/better-language-models/#samples) by GPT-2 from OpenAI's website. The model was given a writing prompt about a scientist discovering unicorns in the Andes Mountains.
...
1. Get an output from GPT-2
2. Append the output to the text preceding it and compare the result to the training text that we want to learn about
3. Slightly adjust GPT-2's parameters so that the next output will be more similar to the training text than the last output
4. Repeat steps 1-3 until we (the humans) decide that GPT-2 has learned enough
...
Additionally, updating the 1.5 billion parameters of GPT-2 requires a lot computation power and time, so I used a [Google Colaboratory document](https://colab.research.google.com/drive/1BajaHZN5HxGTDH_gqk1kZjCfRphUKZp5?usp=sharing) with a GPU (graphics processing unit) to accelerate the training process.

For more technical details on how I fine-tuned GPT-2, I recommending reading through Max Woolf's [excellent guide](https://minimaxir.com/2019/09/howto-gpt2/).
...
Before we continue here, I will warn you that the more modern farewells may be more obviously affected by the bias within GPT-2's original training dataset. More details are in the [conclusion section](#conclusion).
...
You can play around with my code [here](https://colab.research.google.com/drive/1BajaHZN5HxGTDH_gqk1kZjCfRphUKZp5?usp=sharing).
...
Perhaps the _temperature_ was a little high? (Temperature is a text generation parameter: the higher it is, the more "creative" the output becomes.)
